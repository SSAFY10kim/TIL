...
def is_validation(user_list):
    for _ in user_data:
        ...
        if list(user_list["mail"]).count('@') == 0:
            false_list.append("mail")

        if len(list(user_list["name"])) < 2 and len(list(user_list["name"])) > 30:
            false_list.append("name")

        if len(user_list["website"]) == 0:
            false_list.append("website")        # 이상 검증 목록 (false_list)
...
def create_user(is_return):
     
        
    pass

    # blood_group의 값이 blood_types에 포함 되어 있는가
    # company의 값이 black_list에 포함되어 있지 않는가
    # mail의 값에 @ 문자열이 포함되어 있는가
    # name의 값의 길이가 최소 2글자 이상 최대 30글자 이하인가
    # website가 최소 1개 이상 있는가
    # if 하나라도 잘못된 값 있으면 False 반환 + 어떤 데이터가 잘못 되었는가
    # 모두 정상이라면 True
    #
